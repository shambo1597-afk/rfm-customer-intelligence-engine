...
### Two providers — Gemini free tier by default, Anthropic as the paid fallback

The digest can be generated by either of two providers, chosen automatically by `_resolve_provider()` in `src/digest_engine.py`: **Gemini is preferred by default** when a Gemini key is configured; Anthropic is used when only an Anthropic key is present; either key alone is enough to enable the feature. An explicit `DIGEST_PROVIDER` secret/env var (`"gemini"` or `"anthropic"`) can force one, but it silently falls back to whichever key *is* present if the requested provider's key is missing — the feature never hard-fails over a provider preference. Both providers are handed the exact same aggregate-only prompt built by `_build_prompt()` — the PII-safety guarantee above is identical regardless of which one ends up handling the request.
...
| **This design: per account (Gemini, default)** | 1 call *per account*, per batch run | 8 accounts × 1 batch/day | Flash-Lite tier, historically free-tier-eligible — see caveat below |
| **This design: per account (Anthropic, fallback)** | 1 call *per account*, per batch run | 8 accounts × 1 batch/day | **~$1/month** |

All rows use the same tiny prompt size; the ~200× volume-driven cost difference between the rejected design and this one is purely a function of call *granularity* (per-customer vs. per-account), not model choice or prompt size. This design's default provider is Gemini's **Flash-Lite tier**, addressed via `GEMINI_MODEL_ID` in `src/digest_engine.py` — currently `gemini-flash-lite-latest`, Google's own rolling alias for whatever Flash-Lite-tier model it currently recommends, chosen deliberately over pinning one dated model name (see the deprecation incident below) — Google's cheapest, historically free-tier-eligible model family, which is why Anthropic remains a sensible paid fallback rather than the default. **Do not swap the Gemini model for a Pro-tier model** — Pro models require billing to be configured and are not expected to be free-tier eligible. A marketer reviewing their dashboard wants one summary of their whole book of business per day — not one paragraph per customer, which nobody reads end-to-end anyway. The full reasoning (with the underlying call-volume arithmetic) is in `src/digest_engine.py`'s module docstring, so it survives in the code a future contributor actually reads, not just in a planning doc.

> **Pricing caveat:** `src/digest_engine.py`'s module docstring intentionally does *not* restate a specific $/1M-token figure or a firm "genuinely $0" guarantee for whatever model `gemini-flash-lite-latest` currently resolves to — Google's pricing page was unreachable from the environment that made this fix, so no model's specific figures are stated here without verification. Confirm current pricing/free-tier status in Google AI Studio or the live pricing page before treating $0 as guaranteed for a new deployment.
>
> **Model deprecation incident (2026-08-29):** the original default, `gemini-2.5-flash-lite`, started returning `404 NotFoundError` ("no longer available to new users") on real calls, even though it still appeared in `client.models.list()` for the affected project — so the model-listing endpoint alone isn't sufficient to confirm a model is actually callable. Both the Interactions API (`client.interactions.create()`, what this module uses) and the older `client.models.generate_content()` failed identically, then both succeeded identically once switched to a working model — confirming this was a plain model deprecation, not an API-method-specific bug. The immediate fix pinned a specific replacement (`gemini-3.5-flash-lite`); that was then replaced with the `gemini-flash-lite-latest` rolling alias once it was clear pinning a dated model name is exactly what caused the incident, and that replacement model is itself expected to be deprecated in turn. An alias isn't immunity from Google discontinuing the whole Flash-Lite tier, only from needing a code change every time the dated model underneath it rotates. See `src/digest_engine.py`'s module docstring for the full incident note.
...
**Rate limits (Gemini free tier):** Gemini's free-tier rate limits are per-project and shown live in Google AI Studio rather than published as a fixed table. Hitting the free-tier per-minute/per-day ceiling produces a standard `429 RESOURCE_EXHAUSTED` error. This is an **expected operating condition** of this design at higher account counts, not a bug — `generate_account_digest()` catches it and routes to the same fallback template as every other failure mode, never raises, and never breaks the app.
...
- **Gemini free tier:** per Google's own Gemini API pricing page, content sent to the Gemini API **on the free tier is used to improve Google's products** ("Used to improve our products: Yes").
- **Anthropic (paid API):** Anthropic's standard paid-API terms do **not** use submitted content to train models.

This does **not** weaken the PII-safety guarantee above — only aggregate, account-level stats are ever sent to either provider, never raw customer rows, IDs, or names. But it is a real data-handling tradeoff a deployer should weigh deliberately when choosing whether to run the free Gemini tier (or a paid Gemini tier, if configured) versus Anthropic for a given account, not a footnote to skip past.

### Credentials & graceful degradation

Set `GEMINI_API_KEY` and/or `ANTHROPIC_API_KEY` via `st.secrets` or an environment variable — never hardcoded, never logged. Optionally set `DIGEST_PROVIDER` (`"gemini"` or `"anthropic"`) to force a provider. **No key configured is not an error**: `generate_account_digest()` (and every failure path inside it — invalid key, rate limit / `429 RESOURCE_EXHAUSTED`, network error, empty response, missing SDK package) falls back to a deterministic, clearly-labeled template summary built from the same aggregate stats via an f-string, so the feature degrades gracefully and the app never breaks or crashes without a key. Existing users who don't set a key or enable the checkbox see **zero behavior change**.
