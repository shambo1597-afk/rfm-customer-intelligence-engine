...
>
> **Direct-source verification caveat:** `console.groq.com` and `groq.com` were both unreachable from the environment that made this change (network egress blocks — the same restriction that affected the earlier Gemini-pricing investigation). The rate-limit figures below were independently re-verified on 2026-08-30 via three separate, independently-authored third-party sources that converged exactly on the same numbers for `openai/gpt-oss-120b` — 30 RPM, 1,000 RPD, 8,000 TPM, 200,000 TPD — giving reasonable confidence in those specific figures despite not being fetched from Groq's own docs directly. The **data-usage/retention claim** in the section below remains unverified against Groq's own policy pages and should still be confirmed directly before relying on it for any real deployment's compliance posture.
