...
| **This design: per account (Gemini, default)** | 1 call *per account*, per batch run | 8 accounts × 1 batch/day | **~$0/month** (genuinely free tier) |
| **This design: per account (Anthropic, fallback)** | 1 call *per account*, per batch run | 8 accounts × 1 batch/day | **~$1/month** |

All three rows use the same tiny prompt size; the ~200× volume-driven cost difference between the rejected design and this one is purely a function of call *granularity* (per-customer vs. per-account), not model choice or prompt size. On top of that, this design's default provider — **Gemini 2.5 Flash-Lite** (`gemini-2.5-flash-lite`) — is genuinely free of charge on Google's free tier at this call volume, not just cheap; its paid-tier rate ($0.10 / $0.40 per 1M input/output tokens) is also far below Anthropic Haiku 4.5's ($1.00 / $5.00 per 1M input/output tokens), which is why Anthropic remains a sensible fallback rather than the default. **Do not swap the Gemini model for a Pro-tier model** — Pro models require billing to be configured and are not on the free tier. A marketer reviewing their dashboard wants one summary of their whole book of business per day — not one paragraph per customer, which nobody reads end-to-end anyway. The full reasoning (with the underlying call-volume arithmetic) is in `src/digest_engine.py`'s module docstring, so it survives in the code a future contributor actually reads, not just in a planning doc.
