...
--- PII posture: informed exception for two tables, as of this revision ---

UPDATED per an explicit, informed decision -- this is no longer an absolute
"no individual rows, ever" rule. Individual CustomerID-level rows from EXACTLY
TWO tables -- the churn watchlist and the growth targets -- now reach the blob
and the model. Everything else in this file (segment breakdown, cohort
retention, the crosstab, methodology) remains 100% aggregate, unchanged.

Why this is safe for the data in THIS repo, specifically: both datasets
bundled here are either long-published, anonymized academic data (the UCI
Online Retail dataset -- integer CustomerIDs, no real names/emails/addresses,
public since well before this project existed) or synthetic data generated by
this project's own generator (fictional by construction). Sending a
"CustomerID" from either dataset to an LLM API -- Groq's free tier (this
feature's default provider as of 2026-08-29; see digest_engine.py's
data-usage disclosure for the current provider and its sourcing caveat) or
Anthropic's paid tier -- carries no real personal-privacy exposure, because
there is no real person behind either ID, regardless of whichever provider's
own data-handling policy turns out to apply.

>>> THIS REASONING DOES NOT EXTEND TO REAL CUSTOMER DATA. <<< It is scoped
to the two datasets bundled with this repo (data/ecommerce_transactions.csv,
data/real_online_retail.csv[.gz]) and is NOT a blanket "IDs are always fine"
policy. src/shopify_ingest.py can pull LIVE data from a real, connected
Shopify store -- if this pipeline is ever run against a real store's data
(or any other non-synthetic, non-UCI-public source), a real store's
CustomerIDs/order data are NOT the same privacy case as this repo's bundled
datasets, and this design decision (individual rows reaching an LLM API) MUST
be re-evaluated before enabling Chat Q&A against that data. This is a
load-bearing caveat for anyone reusing this codebase against real data, not a
footnote -- see the same caveat, restated, in README's "Chat Q&A (Optional)"
section.

The row cap on each table (WATCHLIST_CHAT_ROW_CAP, GROWTH_TARGET_CHAT_ROW_CAP
below) exists to bound prompt size, NOT for privacy reasons -- the privacy
question for this repo's data is already resolved by the above.

See tests/test_chat_context.py's exposure-scoping test, which asserts BOTH
that watchlist/growth-target CustomerIDs DO appear (the feature works) AND
that a CustomerID belonging to neither table does NOT appear (the exposure
stays scoped to exactly these two tables, not the full customer base).
...
- Churn watchlist: the existing aggregate fields (size, total value, average
  P(Alive), segment composition) of src/clv_engine.py's
  get_urgent_churn_watchlist() output PLUS, as of this revision, up to
  WATCHLIST_CHAT_ROW_CAP individual rows (CustomerID, Monetary, Recency,
  Frequency, Segment, P_Alive_Pct, Churn_Risk_Tier -- exactly the fields that
  function already returns, nothing pulled from rfmt_df beyond it) under
  "individual_rows", already sorted by that function's own priority order
  (highest Monetary first among critical-P(Alive) accounts).
- Growth targets: the existing aggregate fields (size, total predicted 90-day
  value, segment composition) of get_top_future_growth_targets() output PLUS,
  as of this revision, up to GROWTH_TARGET_CHAT_ROW_CAP individual rows
  (CustomerID, Monetary, Recency, Frequency, Segment, Predicted_Spend_90d)
  under "individual_rows", already sorted by that function's own priority
  order (highest Predicted_Spend_90d first).
...
# Individual-row caps for the two tables where CustomerID-level detail now
# reaches the chat context (see module docstring's "PII posture" section) --
# these bound prompt size, NOT privacy exposure (that question is already
# resolved for this repo's bundled datasets). Watchlist rows are already
# sorted by get_urgent_churn_watchlist() (highest Monetary first among
# critical-P(Alive) accounts) and can exceed this cap on a real account (76
# rows on the bundled synthetic dataset); growth-target rows are already
# capped upstream by GROWTH_TARGETS_TOP_N, so this cap is a second, explicit
# backstop rather than a silently-inherited one.
WATCHLIST_CHAT_ROW_CAP = 20
GROWTH_TARGET_CHAT_ROW_CAP = 10
...
def _watchlist_rows_for_chat(watchlist_df, max_rows: int = WATCHLIST_CHAT_ROW_CAP) -> list:
    """
    Serializes up to max_rows individual watchlist rows into small dicts for
    the chat context -- CustomerID + exactly the fields
    get_urgent_churn_watchlist() already returns (Monetary, Recency,
    Frequency, Segment, P_Alive_Pct, Churn_Risk_Tier); nothing pulled from
    rfmt_df or any other source. Rows are taken in the DataFrame's existing
    order (get_urgent_churn_watchlist() already sorts by Monetary descending)
    -- this function does not re-sort. See module docstring's "PII posture"
    section for why individual CustomerIDs are included here at all.
    """
    ...
    return [
        {
            ...
            "p_alive_pct": float(row["P_Alive_Pct"]),
...
def _growth_target_rows_for_chat(growth_df, max_rows: int = GROWTH_TARGET_CHAT_ROW_CAP) -> list:
    """
    Serializes up to max_rows individual growth-target rows into small dicts
    for the chat context -- CustomerID + exactly the fields
    get_top_future_growth_targets() already returns (Monetary, Recency,
    Frequency, Segment, Predicted_Spend_90d); nothing pulled from rfmt_df or
    any other source. Rows are taken in the DataFrame's existing order
    (get_top_future_growth_targets() already sorts by Predicted_Spend_90d
    ...
    return [
        {
            ...
            "frequency": int(row["Frequency"]),
            "segment": str(row["Segment"]),
            "predicted_spend_90d": float(row["Predicted_Spend_90d"]),
        }
...
def build_account_context_blob(
    ...
    Returns a dict of sub-sections (see module docstring's "PII posture"
    section for exactly what each contains -- everything is aggregate-only
    EXCEPT churn_watchlist/growth_targets's "individual_rows", which now
    ...
    churn_watchlist = {
        ...
        "individual_rows": _watchlist_rows_for_chat(watchlist_df),
    }
    ...
    growth_targets = {
        ...
        "individual_rows": _growth_target_rows_for_chat(growth_df),
    }
...
def build_context_text(blob: dict) -> str:
    ...
    written out here comes from the blob dict, nothing derived independently.
    Everything is aggregate EXCEPT the churn-watchlist/growth-target
    "Individual accounts" lines below, which now include CustomerID-level
    detail by informed decision -- see module docstring's "PII posture"
    section for exactly why, and its load-bearing caveat that this does not
    extend to real customer data.
    """
    ...
    watchlist_individual_lines = "\n".join(
        f"- CustomerID {r['customer_id']}: ${r['monetary']:,.2f} historical spend, "
        f"last order {r['recency_days']} days ago, {r['frequency']} orders total, "
        f"Segment: {r['segment']}, P(Alive): {r['p_alive_pct']:.1f}%, Risk tier: {r['churn_risk_tier']}"
        for r in watchlist["individual_rows"]
    ) or "- (no individual accounts to list)"

    growth = blob["growth_targets"]
    growth_composition = ", ".join(
        f"{seg}: {count}" for seg, count in growth["segment_composition"].items()
    ) or "none"
    growth_individual_lines = "\n".join(
        f"- CustomerID {r['customer_id']}: ${r['monetary']:,.2f} historical spend, "
        f"last order {r['recency_days']} days ago, {r['frequency']} orders total, "
        f"Segment: {r['segment']}, predicted 90-day spend: ${r['predicted_spend_90d']:,.2f}"
        for r in growth["individual_rows"]
    ) or "- (no individual accounts to list)"
    ...
    return (
        ...
        f"Individual accounts (sorted by priority -- highest historical spend first):\n"
        f"{watchlist_individual_lines}\n\n"
        ...
        f"Individual accounts (sorted by priority -- highest predicted 90-day spend first):\n"
        f"{growth_individual_lines}\n\n"
